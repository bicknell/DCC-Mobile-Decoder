...
static volatile uint16_t   dcc_bad_preamble      = 0;    // Number of DCC messages too long for our buffer.
#endif
...
void DCC_ISR(void) {
    ...
    if (HAL_GPIO_ReadPin(DCC_IN_GPIO_Port, DCC_IN_Pin)) {
#if DEBUG_PERFORMANCE
        ++dcc_zeros;
#endif
...
void dcc_periodic(void) {
    ...
    if (dcc_per_second == 0) {
        ...
        printf("Perf: %lu idl, %u int, %u drop, %d over, %d pre, %d pkt (%u, %u)\r\n", idle_count, dcc_interrupts, dcc_drops, dcc_overrun, dcc_bad_preamble, dcc_good_packets,
                dcc_zeros, dcc_ones);
        idle_count = 0;
        dcc_interrupts = 0;
        dcc_drops = 0;
